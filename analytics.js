(() => {
  ...
  async function record(listingId,eventType){if(!/^[0-9a-f-]{36}$/i.test(String(listingId||'')))return;const country=String(window.DDH_LOCALIZATION?.state?.country||'').toUpperCase();const headers={apikey:supabaseKey,'Content-Type':'application/json',Prefer:'return=minimal'};try{await fetch(`${supabaseUrl}/rest/v1/rpc/record_listing_event`,{method:'POST',headers,body:JSON.stringify({p_listing_id:listingId,p_visitor_id:visitorId(),p_event_type:eventType,p_country_code:/^[A-Z]{2}$/.test(country)?country:null}),keepalive:true})}catch{}}
