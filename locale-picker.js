(() => {
  ...
  const currencies=()=>{try{return Intl.supportedValuesOf('currency')}catch{return ['USD','EUR','GBP','CAD','AUD','JPY','CNY','INR','AED','NGN','GHS','KES','ZAR','BRL','MXN']}};
  const countries=()=>window.DDH_COUNTRIES?.sorted?.()||[];
  const state=()=>window.DDH_LOCALIZATION?.state||{country:'',currency:'USD'};

  function save(country,currency){
    const payload={country:String(country||'').toUpperCase(),currency:String(currency||'').toUpperCase(),detectedAt:Date.now(),manual:true};
  ...
  function open(){
    ...
    const countryOptions=countries().map(x=>`<option value="${x.code}"${x.code===current.country?' selected':''}>${x.name}</option>`).join('');
    const currencyOptions=[...new Set(currencies())].sort().map(code=>`<option value="${code}"${code===current.currency?' selected':''}>${code} — ${currencyName(code)}</option>`).join('');
    dialog.innerHTML=`<form method="dialog" style="padding:26px"><button value="cancel" aria-label="Close" style="float:right;border:0;background:#eef0f3;border-radius:50%;width:36px;height:36px;font-size:20px;cursor:pointer">×</button><span style="color:#315efb;font-size:12px;font-weight:850;text-transform:uppercase;letter-spacing:.16em">Worldwide marketplace</span><h2 id="localePickerTitle" style="font-size:30px;letter-spacing:-.04em;margin:10px 0 8px">Your region and currency</h2><p style="color:#6e7480;line-height:1.55;margin:0 0 22px">DigitalDeviceHub detects these automatically. You can override them at any time. Seller prices never change; converted prices are estimates for display.</p><label style="display:flex;flex-direction:column;gap:7px;font-size:12px;font-weight:780;margin-bottom:14px">Country / region<select id="localeCountry" style="width:100%;height:46px;border:1px solid #e4e6ea;border-radius:12px;background:#fff;padding:0 13px">${countryOptions}</select></label><label style="display:flex;flex-direction:column;gap:7px;font-size:12px;font-weight:780">Display currency<select id="localeCurrency" style="width:100%;height:46px;border:1px solid #e4e6ea;border-radius:12px;background:#fff;padding:0 13px">${currencyOptions}</select></label><div style="display:flex;gap:10px;flex-wrap:wrap;margin-top:22px"><button id="saveLocale" value="cancel" style="border:0;border-radius:12px;padding:12px 17px;background:#0c0f14;color:#fff;font-weight:780;cursor:pointer">Save preferences</button><button id="autoLocale" value="cancel" style="border:1px solid #e4e6ea;border-radius:12px;padding:12px 17px;background:#fff;color:#0c0f14;font-weight:780;cursor:pointer">Use automatic detection</button></div></form>`;
    document.body.appendChild(dialog);
    dialog.addEventListener('close',()=>dialog.remove());
    dialog.querySelector('#saveLocale').addEventListener('click',e=>{e.preventDefault();save(dialog.querySelector('#localeCountry').value,dialog.querySelector('#localeCurrency').value)});
